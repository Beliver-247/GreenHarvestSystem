...
const CropReadinessList = () => {
    ...
    // Filtered notifications based on search and status filters
    const filteredNotifications = notifications.filter(notification => {
        const isDateMatch = searchDate 
            ? new Date(notification.preferredPickupDate).toLocaleDateString() === new Date(searchDate).toLocaleDateString()
            : true;
    ...
    // Count the total number of notifications (filtered or unfiltered)
                ...
                <ul className="space-y-4">
                    {filteredNotifications.map(notification => (
                        <li
                            ...
                            className="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 flex justify-between items-start"
                        >
                            <div>
